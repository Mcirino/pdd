...
def unpack_words(input_dict, character_offset_begin=None, character_offset_end=None, lemma=None, 
	...
	for i in range(0,len(ziped_tags)):
		...
		#print "~~~~", ziped_tags[i][4]
		#print "####", array_pos
		wordobjs.append(Word(begin_char_offset=ziped_tags[i][0], 
			end_char_offset=ziped_tags[i][1], lemma=ziped_tags[i][2], word=ziped_tags[i][5], pos=ziped_tags[i][3], 
			ner=ziped_tags[i][4], dep_par=dep_tree[i]["parent"], dep_label=dep_tree[i]["label"]))
	return wordobjs
