...
def syn(sents):
	...
	for i in range(0,len(sents)):
		...
		if nextsentid < len(sents):
			...
			if sents[nextsentid].centereds[0] == True and sents[nextsentid].words[0].startswith('Fig'):
				isnextgood = True
		...
		if firstentity != None:
			...
			if (sent.centereds[firstentity.start] == True or isnextgood == True) and \
			   (sent.left_margins[firstentity.start] < 100 or 'genus' in firstentity.type) and len(sent.words) < 20:
			  	...
			  	for j in range(i+1,min(i+100,len(sents))):
					...
					exists_center = False
					for centered in sent2.centereds:
						if centered == True:
							exists_center = True
					...
					for e in sent_2_entities[sentid2]:
						...
						if ('species' in e.type and 'species' in firstentity.type):
							...
							if sent2.centereds[firstentity2.start] == True:
								doesbreak = True
								break

							if sent2.centereds[firstentity2.start] == False and sent2.followeds[firstentity2.start] == True  and (sent2.left_margins[firstentity2.start] < 100 or firstentity_index2 < 3):

								allleft = True
								i = 0
								for w in sent2.words:
									if w == sent2.words[firstentity2.start]: break
									if sent2.boxes[i].left >= sent2.boxes[firstentity2.start].right and sent2.followeds[i] == False:
										allleft = False
									i = i + 1
...
def belongs_to(sents):
	...
	for i in range(0,len(sents)):
		...
		if nextsentid < len(sents):
			nextsent = sents[nextsentid]
			if (nextsent.centereds[0] == True or nextsent.followeds[0]== True) and \
		...
		if flag_start_context == True:
			...
			if firstentity != None:
				...
				for w in sent.words:
					...
					if sent.boxes[j].left >= sent.boxes[firstentity.start].right and sent.centereds[j] == False:
						allleft = False
					j = j + 1

				if (sent.centereds[firstentity.start] == True or isnextgood == True or \
					sent.followeds[firstentity.start] == True) and len(sent.words) < 20 and allleft == True:
		...
		if isgood == True:
			...
			for j in range(i+1, len(goodsents)):
				...
				if isgood2 == True:
					...
					if sents[istenid2].centereds[0] == True:
						break
...
for _row in sys.stdin:
	...
	_properties = row["layout_properties"]
	_boxes = row["layout_boxes"]
	...
	sents = []
	for (docid, sentid, wordidxs, words, poses, ners, lemmas, dep_paths, dep_parents, bounding_boxes, centereds, followeds, left_margins, boxes) in \
		get_sents2(_docid, _sentids, _wordidxs, _words, _poses, _ners, _lemmas, _dep_paths, _dep_parents, _bounding_boxes, _centereds, _followeds, _left_margins, _boxes):
		sent = Sentence(docid, sentid, wordidxs, words, poses, ners, lemmas, dep_paths, dep_parents, bounding_boxes, centereds, followeds, left_margins, boxes)
		sents.append(sent)
